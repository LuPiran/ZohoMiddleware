...
import { columnsFromPropostaBody } from "../utils/formularioColumns.js";
    ...
    if (supabase && recordId) {
      try {
        ...
        if (storageErrors.length) {
          console.error(
            "[PROPOSTA API] Upload de anexos para o bucket Supabase:",
            storageErrors,
          );
        }
        ...
        const { error: dbErr } = await supabase.from("propostas").insert({
          protocolo_portal: numeroProtocolo,
          ...
          ...cadastro,
          ...columnsFromPropostaBody(req.body),
        });
        if (dbErr) {
          console.error(
            "[PROPOSTA API] Erro ao salvar cópia do formulário em propostas:",
            dbErr,
          );
        }
      } catch (persistErr) {
        console.error(
          "[PROPOSTA API] Erro ao persistir formulário no banco local:",
          persistErr,
        );
      }
    }
