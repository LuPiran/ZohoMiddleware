...
export default function MainLayout({ children }) {
  ...
  const { isFaqBlocked } = useFaqOverlay();
  ...
  return (
    <div className="flex min-h-screen w-full flex-1 flex-col bg-tegra-bg-secondary">
      ...
      <BounceFooter />
      {/* Portal: fixed relativo à viewport; fora do motion.div do AnimatedPage (transform quebraria position:fixed). */}
      {!isFaqRoute &&
        !isFaqBlocked &&
        !(showPlatformUpdate && isDashboardRoute) &&
        createPortal(
          <NavLink
            to={ROUTES.FAQ}
            className="fixed bottom-5 right-5 z-[100] inline-flex items-center gap-3 rounded-full border border-white/20 bg-gradient-to-r from-tegra-blue-dark to-tegra-blue px-4 py-3 text-sm font-semibold text-white shadow-[0_14px_32px_rgba(27,52,108,0.28)] transition-all duration-200 hover:-translate-y-0.5 hover:from-tegra-blue hover:to-tegra-blue-dark hover:shadow-[0_18px_36px_rgba(27,52,108,0.36)] focus:outline-none focus:ring-2 focus:ring-tegra-blue-light focus:ring-offset-2 sm:bottom-6 sm:right-6"
